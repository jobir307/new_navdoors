...
@section('content')
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4"><a href="{{ route('dashboard') }}" class="fw-light">Asosiy /</a><span class="text-muted fw-light">Eshik sozlamalari /</span> Doborlar</h4>
    <div class="row">
      <div class="col-md-12">
        <div class="card mb-4">
          ...
          <div class="card-body">
            @if (isset($transom))
              <form method="POST"  action="{{ route('transoms.update', $transom->id) }}">
              @method('PUT')
            @else
              <form method="POST"  action="{{ route('transoms.store') }}">
            @endif
              @csrf
              <div class="row">
                ...
                <div class="mb-3 col-md-2">
                  ...
                  <select class="form-select" id="doortype_id" name="doortype_id">
                    @foreach($doortypes as $key => $value)
                      @if(isset($transom) && $transom->doortype_id == $value->id)
                ...
                <div class="mb-3 col-md-2">
                  ...
                  <input class="form-control" type="number" name="dealer_price" id="dealer_price" autocomplete="off" value="{{ $transom->dealer_price ?? ''  }}" />
                </div>
                <div class="mb-3 col-md-2">
                  <label for="retail_price" class="form-label">Narxi(xaridor uchun)</label>
                  <input class="form-control" type="number" name="retail_price" id="retail_price" autocomplete="off" value="{{ $transom->retail_price ?? ''  }}" />
                </div>
                <div class="mb-3 col-md-1">
                  <label for="installation_price" class="form-label">Ustanovka narxi</label>
                  <input class="form-control" type="text" name="installation_price" id="installation_price" autocomplete="off" value="{{ $transom->installation_price ?? ''  }}" />
                </div>
                <div class="mb-3 col-md-3">
                  <ol style="list-style-type: none;">
                    <label class="form-label">Lavozimlar</label>
                    @foreach($jobs as $key => $value)
                      @if (isset($transom_jobs) && in_array($value->id, $transom_jobs))
          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="table-responsive text-nowrap">
                <table class="table table-bordered table-striped" id="transom_table">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    @foreach($transoms as $key => $value)
                      <tr>
                        ...
                        <td>{{ $value->installation_price ?? 0 }} so'm</td>
                        <td class="text-sm-end">
                          ...
                          <a href="{{ route('transoms.edit', $value->id) }}" class="btn-sm btn btn-icon btn-outline-primary">
