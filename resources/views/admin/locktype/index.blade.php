...
@section('content')
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4"><a href="{{ route('dashboard') }}" class="fw-light">Asosiy /</a><span class="text-muted fw-light">Eshik sozlamalari /</span> Qulf turlari</h4>
    <div class="row">
      <div class="col-md-12">
        <div class="card mb-4">
          ...
          <div class="card-body">
            @if (isset($locktype))
              <form method="POST"  action="{{ route('locktypes.update', $locktype->id) }}">
              @method('PUT')
            @else
              <form method="POST"  action="{{ route('locktypes.store') }}">
            @endif
              @csrf
              <div class="row">
                <div class="mb-3 col-md-4">
                  <label for="name" class="form-label">Mahsulot nomi</label>
                  <input class="form-control" type="text" id="name" name="name" autofocus autocomplete="off" value="{{ $locktype->name ?? ''  }}" />
                </div>
                <div class="mb-3 col-md-4">
                  <label for="dealer_price" class="form-label">Narxi (diler uchun)</label>
                  <input class="form-control" type="number" name="dealer_price" id="dealer_price" autocomplete="off" value="{{ $locktype->dealer_price ?? ''  }}" />
                </div>
                <div class="mb-3 col-md-4">
                  <label for="retail_price" class="form-label">Narxi (xaridor uchun)</label>
                  <input class="form-control" type="number" name="retail_price" id="retail_price" autocomplete="off" value="{{ $locktype->retail_price ?? ''  }}" />
                </div>
              </div>
              <div class="mt-2">
          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="table-responsive text-nowrap">
                <table class="table table-bordered table-striped" id="locktype_table">
                  <thead>
                    <tr>
                      <th style="width: 20px;">T/r</th>
                      <th>Nomi</th>
                      <th>Narxi (diler uchun)</th>
                    ...
                    @foreach($locktypes as $key => $value)
                      <tr>
                        ...
                        <td class="text-sm-end">
                          ...
                          <a href="{{ route('locktypes.edit', $value->id) }}" class="btn-sm btn btn-icon btn-outline-primary">
   ...
   <script type="text/javascript">
    $(document).ready(function() {
      $('body').on('click', '.btn_product_delete', function() {
        let id = $(this).data("id");
        $(".delete_form").attr("action", "locktypes/" + id);
      });

      $("#locktype_table").DataTable({
        "dom": 'rtp',
        "ordering": false
      });
    });
