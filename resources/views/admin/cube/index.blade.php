...
@section('content')
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4"><a href="{{ route('dashboard') }}" class="fw-light">Asosiy /</a><span class="text-muted fw-light">Eshik sozlamalari /</span> Kubik</h4>
    <div class="row">
      <div class="col-md-12">
        <div class="card">
          @if (isset($cube))
            <form method="POST" action="{{ route('cubes.update', $cube->id) }}">
            @method('PUT')
          @else
          ...
          @endif
            ...
            <div class="card-body cube_content">
              @if (!isset($cube))
                <div class="row">
                  <div class="col-md-9"></div>
                  <div class="col-md-3">
                    <button type="button" class="btn btn-outline-success cube_plus" style="float: right; margin-left: 10px;">+</button>
                    <button type="button" class="btn btn-outline-warning cube_minus" style="float: right;">-</button>
                  </div>
                  <div class="col-md-3"></div>
                </div>
              @endif
              <div class="row cube_div mt-2">
                @if (isset($cube))
                  <div class="mb-3 col-md-4">
                    <label for="name" class="form-label">Mahsulot nomi</label>
                    <input class="form-control" id="name" type="text" value="{{ $cube->name }}" name="cubename" autocomplete="off" />
                  </div>
                  <div class="mb-3 col-md-4">
                    <label for="dealer_price" class="form-label">Narxi(diler uchun)</label>
                    <input class="form-control" type="number" name="dealer_price" id="dealer_price" autocomplete="off" value="{{ $cube->dealer_price }}" />
                  </div>
                  <div class="mb-3 col-md-4">
                    <label for="retail_price" class="form-label">Narxi(xaridor uchun)</label>
                    <input class="form-control" type="number" name="retail_price" id="retail_price" autocomplete="off" value="{{ $cube->retail_price }}" />
                  </div>
                @else
                  <div class="mb-3 col-md-4">
                    <label for="name" class="form-label">Mahsulot nomi</label>
                    <input class="form-control" id="name" type="text" name="cubename[]" autocomplete="off" />
                  </div>
                  <div class="mb-3 col-md-4">
                    <label for="dealer_price" class="form-label">Narxi(diler uchun)</label>
        ...
        <div class="row cube_without_labels" style="display:none;">
          <div class="col-md-4">
          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="table-responsive text-nowrap">
                <table class="table table-bordered table-hover" id="cube_table">
                  <thead>
                    <tr>
                    ...
                    @foreach($cubes as $key => $value)
                      <tr>
                        ...
                        <td class="text-sm-end">
                          ...
                          <a href="{{ route('cubes.edit', $value->id) }}" class="btn-sm btn btn-icon btn-outline-primary">
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      $('body').on('click', '.btn_product_delete', function() {
        let id = $(this).data("id");
        $(".delete_form").attr("action", "cubes/" + id);
      });

      let cube_table = $('#cube_table').DataTable({
        dom: 'Qlrtp',
        "ordering": false
      });
      ...
      $('body').on('click', '.cube_plus', function(){
        let cube_content = document.getElementsByClassName("cube_without_labels")[0];
        $('.cube_content').last().append('<div class="row cube_div mb-3">'+cube_content.innerHTML+'</div>');
        i++;
      });

      $('body').on('click', '.cube_minus', function(){
        if(i>1){
          let childDiv = $('body').find('.cube_content .row').last();
          childDiv.remove();
          i--;
        }
      });
