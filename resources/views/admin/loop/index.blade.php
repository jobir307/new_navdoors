...
@section('content')
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4"><a href="{{ route('dashboard') }}" class="fw-light">Asosiy /</a><span class="text-muted fw-light">Eshik sozlamalari /</span> Chaspak</h4>
    <div class="row">
      <div class="col-md-12">
        <div class="card mb-4">
          ...
          <div class="card-body">
            @if (isset($loop))
              <form method="POST"  action="{{ route('loops.update', $loop->id) }}">
              @method('PUT')
            @else
              <form method="POST"  action="{{ route('loops.store') }}">
            @endif
              @csrf
              <div class="row">
                <div class="mb-3 col-md-4">
                  <label for="name" class="form-label">Mahsulot nomi</label>
                  <input class="form-control" type="text" id="name" name="name" autofocus autocomplete="off" value="{{ $loop->name ?? ''  }}" />
                </div>
                <div class="mb-3 col-md-4">
                  <label for="dealer_price" class="form-label">Narxi (diler uchun)</label>
                  <input class="form-control" type="number" name="dealer_price" id="dealer_price" autocomplete="off" value="{{ $loop->dealer_price ?? ''  }}" />
                </div>
                <div class="mb-3 col-md-4">
                  <label for="retail_price" class="form-label">Narxi (xaridor uchun)</label>
                  <input class="form-control" type="number" name="retail_price" id="retail_price" autocomplete="off" value="{{ $loop->retail_price ?? ''  }}" />
                </div>
              </div>
              <div class="mt-2">
          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="table-responsive text-nowrap">
                <table class="table table-bordered table-striped" id="loop_table">
                  <thead>
                    <tr>
                    ...
                    @foreach($loops as $key => $value)
                      <tr>
                        ...
                        <td class="text-sm-end">
                          ...
                          <a href="{{ route('loops.edit', $value->id) }}" class="btn-sm btn btn-icon btn-outline-primary">
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      $('body').on('click', '.btn_product_delete', function() {
        let id = $(this).data("id");
        $(".delete_form").attr("action", "loops/" + id);
      });

      $("#loop_table").DataTable({
        "dom": 'rtp',
        "ordering": false
      });    
    });
