...
@section('content')
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4"><a href="{{ route('dashboard') }}" class="fw-light">Asosiy /</a><span class="text-muted fw-light">Shisha sozlamalari /</span> Shisha turlari</h4>
    <div class="row">
      <div class="col-md-12">
        <div class="card mb-4">
          ...
          <div class="card-body">
            @if (isset($glass_type))
              <form method="POST"  action="{{ route('glass-types.update', $glass_type->id) }}">
              @method('PUT')
            @else
              <form method="POST"  action="{{ route('glass-types.store') }}">
            @endif
              @csrf
              <div class="row">
                <div class="mb-3 col-md-12">
                  <label for="name" class="form-label">Mahsulot nomi</label>
                  <input class="form-control" type="text" id="name" name="name" autofocus autocomplete="off" value="{{ $glass_type->name ?? ''  }}" />
                </div>
              </div>
              <div class="mt-2">
        ...
        @if (isset($glass_types))
          <div class="card">
            <h5 class="card-header">Mahsulotlar ro'yxati</h5>
            <div class="card-body">
              <div class="table-responsive text-nowrap">
                <table class="table table-bordered table-striped w-100" id="glasstype_table">
                  <thead>
                    <tr>
                    ...
                    @foreach($glass_types as $key => $value)
                      <tr>
                        ...
                        <td class="text-sm-end">
                          ...
                          <a href="{{ route('glass-types.edit', $value->id) }}" class="btn-sm btn btn-icon btn-outline-primary">
   ...
   <script type="text/javascript">
    $(document).ready(function() {
      $('body').on('click', '.btn_product_delete', function() {
        let id = $(this).data("id");
        $(".delete_form").attr("action", "glass-types/" + id);
      });

      $("#glasstype_table").DataTable({
        "dom": 'rtp',
        "ordering": false
      });
    });
