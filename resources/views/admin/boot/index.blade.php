...
@section('content')
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4"><a href="{{ route('dashboard') }}" class="fw-light">Asosiy /</a><span class="text-muted fw-light">Eshik sozlamalari /</span> Sapog</h4>
    <div class="row">
      <div class="col-md-12">
        <div class="card">
          @if (isset($boot))
          ...
          @endif
            ...
            <div class="card-body boot_content">
              @if (!isset($boot))
                <div class="row">
                  <div class="col-md-9"></div>
                  <div class="col-md-3">
                    <button type="button" class="btn btn-outline-success boot_plus" style="float: right; margin-left: 10px;">+</button>
                    <button type="button" class="btn btn-outline-warning boot_minus" style="float: right;">-</button>
                  </div>
                  <div class="col-md-3"></div>
                </div>
              @endif
              <div class="row boot_div mt-2">
                @if (isset($boot))
                  <div class="mb-3 col-md-4">
                    <label for="name" class="form-label">Mahsulot nomi</label>
                    <input class="form-control" id="name" type="text" value="{{ $boot->name }}" name="bootname" autocomplete="off" />
                  </div>
                  <div class="mb-3 col-md-4">
                    <label for="dealer_price" class="form-label">Narxi(diler uchun)</label>
                    <input class="form-control" type="number" name="dealer_price" id="dealer_price" autocomplete="off" value="{{ $boot->dealer_price }}" />
                  </div>
                  <div class="mb-3 col-md-4">
                    <label for="retail_price" class="form-label">Narxi(xaridor uchun)</label>
                    <input class="form-control" type="number" name="retail_price" id="retail_price" autocomplete="off" value="{{ $boot->retail_price }}" />
                  </div>
                @else
                  <div class="mb-3 col-md-4">
                    <label for="name" class="form-label">Mahsulot nomi</label>
                    <input class="form-control" type="text" name="bootname[]" id="name" autocomplete="off" />
                  </div>
                  <div class="mb-3 col-md-4">
                    <label for="dealer_price" class="form-label">Narxi(diler uchun)</label>
        ...
        <div class="row boot_without_labels" style="display:none;">
          <div class="col-md-4">
          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="table-responsive text-nowrap">
                <table class="table table-bordered table-hover" id="boot_table">
                  <thead>
                    <tr>
                    ...
                    @foreach($boots as $key => $value)
                      <tr>
                        ...
                        <td class="text-sm-end">
                          ...
                          <a href="{{ route('boots.edit', $value->id) }}" class="btn-sm btn btn-icon btn-outline-primary">
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      $('body').on('click', '.btn_product_delete', function() {
        let id = $(this).data("id");
        $(".delete_form").attr("action", "boots/" + id);
      });

      let cube_table = $('#boot_table').DataTable({
        dom: 'Qlrtp',
        "ordering": false
      });
      ...
      $('body').on('click', '.boot_plus', function(){
        let boot_content = document.getElementsByClassName("boot_without_labels")[0];
        $('.boot_content').last().append('<div class="row boot_div mb-3">'+boot_content.innerHTML+'</div>');
        i++;
      });

      $('body').on('click', '.boot_minus', function(){
        if(i>1){
          let childDiv = $('body').find('.boot_content .row').last();
          childDiv.remove();
          i--;
        }
      });
