...
@section('content')
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4"><a href="{{ route('dashboard') }}" class="fw-light">Asosiy /</a><span class="text-muted fw-light">Eshik sozlamalari /</span> Qalinliklar</h4>
    <div class="row">
      <div class="col-md-12">
        <div class="card mb-4">
          ...
          <div class="card-body">
            @if (isset($depth))
              <form method="POST"  action="{{ route('depths.update', $depth->id) }}">
              @method('PUT')
            @else
              <form method="POST"  action="{{ route('depths.store') }}">
            @endif
              @csrf
              <div class="row">
                <div class="mb-3 col-md-6">
                  <label for="name" class="form-label">Mahsulot nomi</label>
                  <input class="form-control" type="text" id="name" name="name" autofocus autocomplete="off" value="{{ $depth->name ?? ''  }}" />
                </div>
                <div class="mb-3 col-md-6">
                  <label for="price" class="form-label">Narxi</label>
                  <input class="form-control" type="number" name="price" id="price" autocomplete="off" value="{{ $depth->price ?? ''  }}" />
                </div>
              </div>
              <div class="mt-2">
          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="table-responsive text-nowrap">
                <table class="table table-bordered table-striped" id="depth_table">
                  <thead>
                    <tr>
                    ...
                    @foreach($depths as $key => $value)
                      <tr>
                        ...
                        <td class="text-sm-end">
                          ...
                          <a href="{{ route('depths.edit', $value->id) }}" class="btn-sm btn btn-icon btn-outline-primary">
   ...
   <script type="text/javascript">
    $(document).ready(function() {
      $('body').on('click', '.btn_product_delete', function() {
        let id = $(this).data("id");
        $(".delete_form").attr("action", "depths/" + id);
      });
