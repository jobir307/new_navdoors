...
<table class="table table-bordered w-100">
    <tbody>
        ...
        <tr>
            ...
            <td>Shartnoma raqami: {{ $order[0]->id }}/{{ $order[0]->contract_number }}({{ $nsjambs[0]->nsjamb_name }} {{ $nsjambs[0]->nsjamb_color }})</td>
        </tr>
        <tr>
    ...
    <tbody>
        @if (!is_null($nsjambs))
            @foreach($nsjambs as $key => $value)
                @if (!empty($value->nsjamb_name))
                    <tr>
                        <td class="text-center align-middle">{{ $key + 1 }}</td>
                        <td class="align-middle">{{ $value->nsjamb_name }}x{{ $value->height }}x{{ $value->width_top }}x{{ $value->width_bottom }}</td>
